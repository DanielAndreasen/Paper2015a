...
exist sevaral methods to derive these parameters, and with up-coming NIR
spectrographs, we will push for this spectral region.}
% Aims
...
ionization balance, assuming Kurucz ATLAS9 model atmospheres in LTE.
The abundance analysis was determined using the code, MOOG. We use a
high resolution and high signal-to-noise ratio spectrum of the Sun as a
starting point to compile the iron line list. We tested the line list on
HD20010, and the result indicate a necessary cut in excitation potential
of the line list.}
% Results
...
was fixed duing the proccess. The rest of the stellar atmospheric
...
mass and radius \citep[e.g.][]{Kjeldsen1995}. This technique has
...
below 20000, it is preferred to fit the overall observed spectrum with
a synthetic one \citep[see e.g.][]{Recio2006}. For higher resolution
at low rotational velocities (below 10 to 15 \si{km/s}) we can use the
equivalent width (EW) method (for details see Sect.~\ref{sec:method}).
For FGK stars it is preferable to use optical spectra since these stars
have the peak flux in this wavelength domain. However, when we move to
the cooler M stars the near infrared (NIR) is preferred because of the
higher brightness and lower presence of molecular lines in the spectra.
...
w.html}}, as well as the IRD spectrograph
installed at Subaru \citep{IRD}. Furthermore, the
CRIRES spectrograph at VLT is being updated to
CRIRES+\footnote{\url{http://www.eso.org/sci/facilities/develop/instrume
nts/crires_up.html}}, CARMENES for the 3.5 m telescope at
Calar Alto Observatory \citep{CARMENES}, and SPIRou at
...
In this paper we want to explore the possibility to create a line
list of iron lines in the NIR which can be applied for FGK stars in
a consistent way as it is currently done for FGK stars. The paper is
organized as follows: In Sect.~\ref{sec:method} we present the method
for deriving parameters with the equivalent width method for an iron
line list. In Sect.~\ref{sec:results} we present the results for the
derived parameters for the Sun and HD20010.
...
At this point we ended up with 594 and 15 lines of FeI and FeII,
...
synthesis. Here the metallicity was set to [Fe/H]=0 for all
...
Fig~\ref{fig:Fe1_before_recal} the EWs of the Sun of the Fe I
lines are plotted with respect to the excitation potential. This
plot shows the distribution before recalibration of $\log gf$ and
...
draws in total for both SNR considered. From each new line list we
applied a cut in excitation potential at \SI{5.0}{eV} and \SI{5.5}{eV}
resulting in a total of 61 line lists. For each SNR we consider we have 30
line lists in addition to the original line list. We make the cut in excitation
potential after we tried to derive parameters for HD20010 without any cuts in
excitation potential and overestimated the parameters (see more in
Sect~\ref{sec:derived_parameters_of_hd20010}). With different cuts in EP
we were able to get more reliable parameters. Hence we applied the cuts for
the Sun as well in order to see the effect of the derived parameters.
...
The color shows the two different SNR considered (blue is SNR at 100,
and green is SNR at 300). For a given SNR and EP cut, the 10 runs were
combined to a single value by calculating the mean. Additionally,
we show the 1$\sigma$ standard deviation. A few of the runs did not
converge with our minimization routine. For these with manual changed
the atmospheric model parameters untill we met the convergence criteria.
...
after we apply a cut in excitation potential around \SI{5.5}{eV}.

The line list still remains to be tested for a larger sample of other FGK
stars, but as important for M stars. Currently, there is one M dwarf in the
CRIRES-POP, Barnard's star which will be ideal for testing in the future.

Moreover, with the up-coming NIR spectrographs, this work and future
continuation will help the community to derive atmospheric stellar parameters.
