...
With the lack of \ion{Fe}{ii} lines from the
...
is usually considered as one of the most reliable methods for FGK dwarf
and subgiant stars. However, the IRFM needs a priori knowledge of the bolometric
flux, surface gravity and stellar metallicity
\citep{Blackwell1977,Ramirez2005b,Casagrande2010}.

Finally, the use of high resolution spectroscopy along with stellar
atmospheric models is an extensively tested method that allows the
derivation of the fundamental parameters of a star \citep[see e.g.][]{Santos13}
The procedure depends on the quality of the
...
stars (below 10 to 15 \si{km/s}) we are in the regime where the equivalent
width (EW) method can be used (for details see Sect.~\ref{sec:method}).
...
wavelength domain. At the moment the GIANO spectrograph installed at TNC is already
...
to an observed spectrum and finds the best model by a minimization procedure
...
The equivalent width (EW) method \citep{Sousa2008a,Bensby2014,Mucciarelli2013},
which we use in this work, is based on the measurements
of EWs of a given line list of atomic data of absorption lines visible in the
stellar spectrum of interest. The EW for a single line is given as:
\begin{align}
    \label{eq:EW}
...
Using the EW method, the abundance for individual lines can be calculated
...
single ionized iron: \ion{Fe}{i} and \ion{Fe}{ii}, respectively, which are also used to
...
In total 78537 lines (\ion{Fe}{i}: 50198 and \ion{Fe}{ii}: 28339) are present in
...
we reduced the number of lines to 6060 and 2735 for \ion{Fe}{i} and \ion{Fe}{ii},
respectively.
...
we were down to 593 \ion{Fe}{i} lines and 22 \ion{Fe}{ii} lines.
...
At this point we ended up with 593 and 12 lines of \ion{Fe}{i} and \ion{Fe}{ii},
...
iron abundance of more than 1 dex. Note that we only removed \ion{Fe}{i} lines
here, since the \ion{Fe}{ii} lines are sparse and essential to determine the
...
are down to 319 and 12 lines, for \ion{Fe}{i} and \ion{Fe}{ii} respectively.
...
solar value. The distribution of \ion{Fe}{i} and \ion{Fe}{ii} lines are shown in
...
ionization balance, i.e. the mean abundance of \ion{Fe}{i} and \ion{Fe}{ii} are equal.
...
difference of less than 0.005 between the mean abundances for \ion{Fe}{i} and
\ion{Fe}{ii}.
...
wavelength calibration at the moment. We were able to measure 249 \ion{Fe}{i}
lines and 5 \ion{Fe}{ii} lines compared to 344 \ion{Fe}{i} lines and 13 \ion{Fe}{ii} lines for
the Sun.
...
any of the very few remaining \ion{Fe}{ii} lines. The reason not to remove
the \ion{Fe}{ii} lines is, that they are essential to determine the surface
